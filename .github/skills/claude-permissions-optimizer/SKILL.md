...
Then proceed to Step 1 normally. The skill works from any environment as long as `~/.copilot/` (or `$CLAUDE_CONFIG_DIR`) exists on the machine.
...
1. **Apply all to user settings** (`~/.copilot/settings.json`)
2. **Apply all to project settings** (`.github/settings.json`)
3. **Skip**
...
```
Applied N rules to ~/.copilot/settings.json
Applied M rules to .github/settings.json

These commands will no longer trigger permission prompts.
```

If `.github/settings.json` was modified and is tracked by git, mention that committing it would benefit teammates.

## Edge Cases

- **No project context** (running outside a project): Only offer user-level settings as write target.
- **Settings file doesn't exist**: Create it with `{ "permissions": { "allow": [] } }`. For `.github/settings.json`, also create the `.github/` directory if needed.
